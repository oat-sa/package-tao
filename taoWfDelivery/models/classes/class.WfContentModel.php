	...
	implements taoDelivery_models_classes_ContentModel
{
    ...
    public function onChangeDeliveryLabel( core_kernel_classes_Resource $delivery) {
        $content = taoDelivery_models_classes_DeliveryService::singleton()->getContent($delivery);
        if (is_null($content)) {
    ...
    /**
     * (non-PHPdoc)
     * @see taoDelivery_models_classes_ContentModel::getCompiler()
     */
    public function getCompiler( core_kernel_classes_Resource $content) {
        return  new taoWfDelivery_models_classes_DeliveryCompiler($content);
    }
