	...
	qti_initParam["<?=$serial?>"]['matchMaxes'] = {
		<?$i=0;foreach($choices as $choice):?>
			<?=$choice->getIdentifier()?>: { 
				...
				matchGroup	: <?=($choice->getAttributeValue('matchGroup')) ? (is_array($choice->getAttributeValue('matchGroup'))) ? json_encode(implode(' ',$choice->getAttributeValue('matchGroup'))) : "['".$choice->getAttributeValue('matchGroup')."']" : "[]"?>,
				current		: "0"
			}<?=($i<count($choices)-1)?',':''?>
		<?$i++;endforeach?>
	};
