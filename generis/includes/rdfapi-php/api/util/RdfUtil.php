...
class RDFUtil extends Object {
	...
	*/
	function getPrefix($uri) {
	    ...
	    {
                case _RDF_NAMESPACE_URI:
                $prefix = _RDF_NAMESPACE_PREFIX;
                break;

                case _RDF_SCHEMA_URI:
                $short_p = _RDF_SCHEMA_PREFIX;
                break;
	...
	*/
	function isRDF($resource) {
		return ($resource != NULL && RDFUtil::getNamespace($resource) == _RDF_NAMESPACE_URI);
	}
	...
	*/
	function createOrd($num)  {
		return new Resource(_RDF_NAMESPACE_URI . '_' . $num);
	}
	...
	*/
	function chooseColor($node)  {
		...
		return HTML_TABLE_LITERAL_COLOR;
			...
			if (RDFUtil::getNamespace($node) == _RDF_NAMESPACE_URI ||
			RDFUtil::getNamespace($node) == _RDF_SCHEMA_URI ||
			RDFUtil::getNamespace($node) == OWL_URI
			)

			return HTML_TABLE__RDF_NS_COLOR;
		}
