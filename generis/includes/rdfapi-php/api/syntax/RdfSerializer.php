...
class RdfSerializer extends Object {
	...
	*/
	function & serialize(&$model, $xml_default_namespace = NULL, $encoding = DEFAULT_ENCODING) {
		...
		if ($xml_default_namespace) {
			...
			if ($xml_default_namespace == _RDF_NAMESPACE_URI) {
				$this->rdf_qnames = FALSE;
				unset($this->m_defaultNamespaces[_RDF_NAMESPACE_PREFIX]);
			}
			elseif ($xml_default_namespace == _RDF_SCHEMA_URI) {
				unset($this->m_defaultNamespaces[_RDF_SCHEMA_PREFIX]);
			}
		...
		if ($this->rdf_qnames){
			if(isset($nsps[_RDF_NAMESPACE_URI])){
				$this->rdf_qname_prefix = $nsps[_RDF_NAMESPACE_URI].':';
			}else{
				$this->rdf_qname_prefix = _RDF_NAMESPACE_PREFIX . ':';
			}
		...
		if ($model->size() == 0) return "<". $this->rdf_qname_prefix . _RDF_RDF ." xmlns:rdf='"._RDF_NAMESPACE_URI."' />";
		...
		// start the RDF text
		$this->m_out .= '<' . $this->rdf_qname_prefix . _RDF_RDF;
	...
	*/
	function writeNamespaceDeclarations() {
		foreach($this->m_namespaces as $prefix => $namespace) {
			...
			if ($prefix == _RDF_NAMESPACE_PREFIX && !$this->rdf_qnames) {
	...
	*/
	function writeDescriptions()  {
		...
		foreach($this->m_statements as $key => $statement) {
            ...
            if (($predicate->getURI() == _RDF_NAMESPACE_URI._RDF_TYPE) && is_a($object, 'Resource') && !$this->m_groupTypeStatement) {
                $this->writeGroup();
	...
	*/
	function writeGroup() {
		...
		$outerElementName = $this->rdf_qname_prefix . _RDF_DESCRIPTION;
		$this->m_out .= LINEFEED . '<';
	...
	*/
	...
	function writeSubjectURI($subject_node) {
		...
		if (is_a($this->m_currentSubject, 'BlankNode')) {
			$this->m_out .= $this->rdf_qname_prefix . _RDF_NODEID;
			$this->m_out .= '="';
			...
			if (!($relativizedURI == $currentSubjectURI)) {
				$this->m_out .= $this->rdf_qname_prefix . _RDF_ID;
				$this->m_out .= '="';
				$this->m_out .= $relativizedURI;
			} else {
				$this->m_out .= $this->rdf_qname_prefix . _RDF_ABOUT;
				$this->m_out .= '="';
		...
		foreach($this->m_contentStatements as $key => $statement) {
			...
			if (is_a($statement->obj, 'Resource')) {
				...
				if(is_a($statement->obj, 'Literal')) {
					...
					if ($statement->obj->getDatatype()== _RDF_NAMESPACE_URI . _RDF_XMLLITERAL) {
						$this->m_out .= ' ' . _RDF_NAMESPACE_PREFIX . ':' . _RDF_PARSE_TYPE . '="' . _RDF_PARSE_TYPE_LITERAL . '"';
					} else {
						$this->m_out .= ' ' . _RDF_NAMESPACE_PREFIX . ':' . _RDF_DATATYPE . '="' . $statement->obj->getDatatype() . '"';
					}
	...
	*/
	function writeResourceReference($object_node)  {
		...
		if (is_a($object_node, 'BlankNode')) {
			...
			$this->m_out .= $this->rdf_qname_prefix . _RDF_RESOURCE;
		};
	...
	function collectNamespaces($model)  {
		foreach($model->triples as $key => $value) {
			...
			if ($this->use_entities) {
				...
				if  ($value->pred->getURI() == _RDF_NAMESPACE_URI._RDF_TYPE)
				$this->collectNamespace($value->getObject());
				elseif
				(($value->pred->getURI() == _RDF_NAMESPACE_URI._RDFS_SUBCLASSOF) ||
				($value->pred->getURI() == _RDF_NAMESPACE_URI._RDFS_SUBPROPERTYOF)) {
					$this->collectNamespace($value->getSubject());
					$this->collectNamespace($value->getObject());
				}
			}
