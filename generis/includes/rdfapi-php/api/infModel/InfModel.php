...
class InfModel extends MemModel 
{
    ...
    */	
	function InfModel ($baseURI = NULL) 
	{
		...
		if (INF_RES_SUBCLASSOF)
			$this->supportedInference[] = _RDF_SCHEMA_URI._RDFS_SUBCLASSOF;
			
		if (INF_RES_SUBPROPERTYOF)
			$this->supportedInference[] = _RDF_SCHEMA_URI._RDFS_SUBPROPERTYOF;

		if (INF_RES_RANGE)
			$this->supportedInference[] = _RDF_SCHEMA_URI._RDFS_RANGE;

		if (INF_RES_DOMAIN)
			$this->supportedInference[] = _RDF_SCHEMA_URI._RDFS_DOMAIN;
		...
		{
			$infRule=new InfRule();
			$infRule->setTrigger(null,new Resource(_RDF_NAMESPACE_URI._RDF_TYPE),new Resource(_RDF_SCHEMA_URI.'ContainerMembershipProperty'));
			$infRule->setEntailment('<s>',new Resource(_RDF_SCHEMA_URI._RDFS_SUBPROPERTYOF),new Resource(_RDF_SCHEMA_URI.'member'));
		...
		{
			$infRule=new InfRule();
			$infRule->setTrigger(null,new Resource(_RDF_NAMESPACE_URI._RDF_TYPE),new Resource(_RDF_NAMESPACE_URI._RDF_PROPERTY));
			$infRule->setEntailment('<s>',new Resource(_RDF_SCHEMA_URI._RDFS_SUBPROPERTYOF),'<s>');
			$this->_addInfRule($infRule,'base');
		}
		...
		{
			$infRule=new InfRule();
			$infRule->setTrigger(null,new Resource(_RDF_NAMESPACE_URI._RDF_TYPE),new Resource(_RDF_SCHEMA_URI._RDFS_CLASS));
			$infRule->setEntailment('<s>',new Resource(_RDF_SCHEMA_URI._RDFS_SUBCLASSOF),new Resource(_RDF_SCHEMA_URI._RDFS_RESOURCE));
			$this->_addInfRule($infRule,'base');
		...
		if (INF_RES_RULE_RDFS10)
		{
			$infRule=new InfRule();
			$infRule->setTrigger(null,new Resource(_RDF_NAMESPACE_URI._RDF_TYPE),new Resource(_RDF_SCHEMA_URI._RDFS_CLASS));
			$infRule->setEntailment('<s>',new Resource(_RDF_SCHEMA_URI._RDFS_SUBCLASSOF),'<s>');
			$this->_addInfRule($infRule,'base');
		}
		...
		{
			$infRule=new InfRule();
			$infRule->setTrigger(null,new Resource(_RDF_NAMESPACE_URI._RDF_TYPE),new Resource(_RDF_SCHEMA_URI._RDFS_DATATYPE));
			$infRule->setEntailment('<s>',new Resource(_RDF_SCHEMA_URI._RDFS_SUBCLASSOF),new Resource(_RDF_SCHEMA_URI._RDFS_LITERAL));
			$this->_addInfRule($infRule,'base');
		}
	...
	*
	...
	function _addToInference($statement)
	{
		...
		{
			case _RDF_SCHEMA_URI._RDFS_SUBPROPERTYOF :
				//create a new rule
				$infRule=new InfRule();
			...
			case _RDF_SCHEMA_URI._RDFS_SUBCLASSOF :
				$infRule=new InfRule();
				$infRule->setTrigger(null,new Resource(_RDF_NAMESPACE_URI._RDF_TYPE),$statement->getSubject());
				$infRule->setEntailment('<s>',new Resource(_RDF_NAMESPACE_URI._RDF_TYPE),$statement->getObject());
				$this->infRules[]=$infRule;
				$this->_addInfRule($infRule,$statementPosition);
			break;
			
			case _RDF_SCHEMA_URI._RDFS_DOMAIN :
				$infRule=new InfRule();
				$infRule->setTrigger(null,$statement->getSubject(),null);
				$infRule->setEntailment('<s>',new Resource(_RDF_NAMESPACE_URI._RDF_TYPE),$statement->getObject());
				$this->infRules[]=$infRule;
			...
			case _RDF_SCHEMA_URI._RDFS_RANGE :
				$infRule=new InfRule();
				$infRule->setTrigger(null,$statement->getSubject(),null);
				$infRule->setEntailment('<o>',new Resource(_RDF_NAMESPACE_URI._RDF_TYPE),$statement->getObject());
				$this->infRules[]=$infRule;
				$this->_addInfRule($infRule,$statementPosition);
			break;
