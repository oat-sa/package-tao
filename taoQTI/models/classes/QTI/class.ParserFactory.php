...
class taoQTI_models_classes_QTI_ParserFactory
{
    ...
    protected function replaceNode(DOMElement $node, taoQTI_models_classes_QTI_Element $element){
    ...
    protected function parseContainerStatic(DOMElement $data, taoQTI_models_classes_QTI_container_Container $container){
        ...
        //parse for rubricBlocks
        $rubricNodes = $this->queryXPath(".//*[name(.)='rubricBlock']", $data);
        foreach($rubricNodes as $rubricNode){
            //@todo: to be implemented
            throw new taoQTI_models_classes_QTI_UnsupportedQtiElement('rubricBlock');
            break;
            //@todo : rubric node actually exists only in item body, change that!
        ...
        //@todo: parse for printedVariables.
        $printedVariableNodes = $this->queryXPath(".//*[name(.)='printedVariable']", $data);
        foreach($printedVariableNodes as $printedVariableNode){
            //@todo: to be implemented
            throw new taoQTI_models_classes_QTI_UnsupportedQtiElement('printedVariable');
            break;
        }

        //finally, add all body elements to the body
        $bodyData = $this->getBodyData($data);
        $bodyData = taoQTI_helpers_qti_ItemAuthoring::cleanHTML($bodyData);
        
        if(empty($bodyElements)){
            $container->edit($bodyData);
        }elseif(!$container->setElements($bodyElements, $bodyData)){
            var_dump($container->toArray(), $bodyElements, $bodyData);
            throw new taoQTI_models_classes_QTI_ParsingException('Cannot set elements to the static container');
        }
    ...
    protected function getAncestors(DOMElement $data, $topNode = 'itemBody'){
        ...
        while(!is_null($currentNode->parentNode) && $parentNodeName != $topNode){
            ...
            if($i > 100){
                throw new taoQTI_models_classes_QTI_ParsingException('maximum recursion of 100 reached');
            }
    ...
    protected function parseContainerInteractive(DOMElement $data, taoQTI_models_classes_QTI_container_ContainerInteractive $container){
        ...
        if(!$container->setElements($bodyElements, $bodyData)){
            throw new taoQTI_models_classes_QTI_ParsingException('Cannot set elements to the interactive container');
        }

        return $this->parseContainerStatic($data, $container);
    }

    private function parseContainerChoice(DOMElement $data, taoQTI_models_classes_QTI_container_Container $container, $tag){
    ...
    protected function parseContainerGap(DOMElement $data, taoQTI_models_classes_QTI_container_ContainerGap $container){
        return $this->parseContainerChoice($data, $container, 'gap');
    }

    protected function parseContainerHottext(DOMElement $data, taoQTI_models_classes_QTI_container_ContainerHottext $container){
     ...
     * @throws taoQTI_models_classes_QTI_ParsingException
     */
    protected function buildItem(DOMElement $data){
        ...
        $this->item = new taoQTI_models_classes_QTI_Item($this->extractAttributes($data));
        $namespaces = array();
        ...
        if($itemBodies === false){
            ...
            $this->parseContainerInteractive($itemBodies->item(0), $this->item->getBody());
        }


        //warning: extract the response processing at the latest to make taoQTI_models_classes_QTI_response_TemplatesDriven::takeOverFrom() work
    ...
    protected function buildInteraction(DOMElement $data){
        ...
        try{
            ...
            $interactionClass = 'taoQTI_models_classes_QTI_interaction_'.$type;
            if(!class_exists($interactionClass)){
                throw new taoQTI_models_classes_QTI_ParsingException('The interaction class cannot be found: '.$interactionClass);
            }

            $myInteraction = new $interactionClass($this->extractAttributes($data), $this->item);

            if($myInteraction instanceof taoQTI_models_classes_QTI_interaction_BlockInteraction){
                //extract prompt:
            ...
            if($myInteraction instanceof taoQTI_models_classes_QTI_interaction_ObjectInteraction){
    ...
    protected function buildChoice(DOMElement $data){
        ...
        $className = 'taoQTI_models_classes_QTI_choice_'.ucfirst($data->nodeName);
        if(!class_exists($className)){
            throw new taoQTI_models_classes_QTI_ParsingException("The choice class does not exist ".$className);
        }

        $myChoice = new $className($this->extractAttributes($data));

        if($myChoice instanceof taoQTI_models_classes_QTI_choice_ContainerChoice){
            $this->parseContainerStatic($data, $myChoice->getBody());
        }elseif($myChoice instanceof taoQTI_models_classes_QTI_choice_TextVariableChoice){
            $myChoice->setContent($data->nodeValue);
        }elseif($myChoice instanceof taoQTI_models_classes_QTI_choice_GapImg){
    ...
    protected function buildResponseDeclaration(DOMElement $data){
        ...
        $myResponse = new taoQTI_models_classes_QTI_ResponseDeclaration($this->extractAttributes($data), $this->item);
    ...
    protected function buildOutcomeDeclaration(DOMElement $data){
        ...
        $outcome = new taoQTI_models_classes_QTI_OutcomeDeclaration($this->extractAttributes($data));
        $data = simplexml_import_dom($data);
     ...
     * @return taoQTI_models_classes_QTI_response_ResponseProcessing
     */
    protected function buildResponseProcessing(DOMElement $data, taoQTI_models_classes_QTI_Item $item){
        $returnValue = null;
        ...
        if(is_null($returnValue)){
            try{
                $returnValue = $this->buildCompositeResponseProcessing($data, $item);
     ...
     * @return taoQTI_models_classes_QTI_response_ResponseProcessing
     */
    protected function buildCompositeResponseProcessing(DOMElement $data, taoQTI_models_classes_QTI_Item $item){
        $returnValue = null;
    ...
    protected function buildExpression(DOMElement $data){
        $data = simplexml_import_dom($data);
        return taoQTI_models_classes_QTI_expression_ExpressionParserFactory::build($data);
    }
    ...
    protected function getModalFeedback($identifier){
        ...
        throw new taoQTI_models_classes_QTI_ParsingException('cannot found the modal feedback with identifier '.$identifier);
    }
    ...
    protected function getOutcome($identifier){
        ...
        throw new taoQTI_models_classes_QTI_ParsingException('cannot found the outcome with identifier '.$identifier);
    }
    ...
    protected function getResponse($identifier){
        ...
        throw new taoQTI_models_classes_QTI_ParsingException('cannot found the response with identifier '.$identifier);
    }
        ...
        foreach($data as $responseRule){
            ...
            if(count($subtree->xpath($patternCorrectTAO)) > 0){
                ...
                try{
                    ...
                    $feedbackRule = new taoQTI_models_classes_QTI_response_SimpleFeedbackRule($outcome, $feedbackThen, $feedbackElse);
                    ...
                    throw new taoQTI_models_classes_QTI_UnexpectedResponseProcessingException('Feedback resources not found. Not template driven, unknown rule');
                }
    ...
    private function buildObject(DOMElement $data){
        ...
        $returnValue = new taoQTI_models_classes_QTI_Object($attributes);
    ...
    private function buildMath(DOMElement $data){
        ...
        $math = new taoQTI_models_classes_QTI_Math($this->extractAttributes($data));
    ...
    private function buildStylesheet(DOMElement $data){
        $returnValue = new taoQTI_models_classes_QTI_Stylesheet(array(
    ...
    private function buildRubricBlock(DOMElement $data){
        ...
        $returnValue = new taoQTI_models_classes_QTI_RubricBlock($this->extractAttributes($data));
    ...
    private function buildFeedback(DOMElement $data, $interactive = false){
        ...
        $type = ucfirst($data->nodeName);
        $feedbackClass = 'taoQTI_models_classes_QTI_feedback_'.$type;
        if(!class_exists($feedbackClass)){
            throw new taoQTI_models_classes_QTI_ParsingException('The interaction class cannot be found: '.$feedbackClass);
        }
        ...
        if($data->nodeName == 'modalFeedback'){
            ...
            throw new taoQTI_models_classes_QTI_UnsupportedQtiElement($type);

            if($interactive){
                $container = new taoQTI_models_classes_QTI_container_ContainerInteractive();
                $myFeedback = new $feedbackClass($container, $attributes, $this->item);
                $this->parseContainerInteractive($data, $myFeedback->getBody());
            }else{
                $container = new taoQTI_models_classes_QTI_container_ContainerStatic();
                $myFeedback = new $feedbackClass($container, $attributes, $this->item);
                $this->parseContainerStatic($data, $myFeedback->getBody());
            }
        }
