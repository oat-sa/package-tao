 ...
 */
class taoQTI_actions_QtiItemRunner extends taoItems_actions_ItemRunner
{

    /**
     * Get the result server API call.
     * 
     * @throws common_Exception If no compilation folder is found.
     */
    protected function getResultServerApi(){
        if ($this->hasRequestParameter('itemPath')) {
            return taoQTI_helpers_ResultServerJsApi::getServiceApi(new core_kernel_file_File($this->getRequestParameter('itemPath')));
        }
        else {
            throw new common_Exception("No 'itemPath' request parameter provided.");
        }
    }

    protected function getResultServerApiPath(){
        return 'taoQTI/views/js/QtiResultServerApi.js';
    }
