QtiWidget.DefaultWidget.GraphicAssociateInteraction = QtiWidget.DefaultWidget.GraphicInteraction.extend({
    render : function(){
        ...
        for(var choiceId in ctx.options.graphicChoices){
            ...
            switch(currentHotSpotShape){
                ...
                case "ellipse":
                    ...
                    break;
                case "poly":
                    polyCoords = QtiWidget.DefaultWidget.polyCoordonates(ctx.options.graphicChoices[choiceId]["coords"]);
                    currentShape = paper.path(polyCoords);

                    if(ctx.graphicDebug)
                        currentShape.attr("stroke-width", "3px");

                    var polyCenter = QtiWidget.DefaultWidget.getShapeCenter(currentShape);
