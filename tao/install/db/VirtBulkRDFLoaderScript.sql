...
create procedure ld_dir (in path varchar, in mask varchar, in graph varchar)
{
  declare ls any;
  declare inx int;
  ...
  for (inx := 0; inx < length (ls); inx := inx + 1)
    {
      if (ls[inx] like mask and not (ls[inx] like 'local.rdf'))
	{
...
create procedure rdf_read_dir (in path varchar, in mask varchar, in graph varchar)
{
...
create procedure ld_add (in _fname varchar, in _graph varchar)
{
...
create procedure ld_file (in f varchar, in graph varchar)
{
...
create procedure rdf_loader_run (in max_files integer := null, in log_enable int := 2)
{
...
create procedure empty_list ()
{
        delete from DB.DBA.LOAD_LIST;
}
;

create procedure loadTAOontology (in f varchar)
{
        empty_list();

        ld_dir(f || '/generis/core/ontology', '*.rdf', 'http://tao.ontology/generis');
        ld_dir(f || '/tao/models/ontology', '*.rdf', 'http://tao.ontology/tao');
        ld_dir(f || '/taoDelivery/models/ontology', '*.rdf', 'http://tao.ontology/taoDelivery');
        ld_dir(f || '/taoItems/models/ontology', '*.rdf', 'http://tao.ontology/taoItems');
        ld_dir(f || '/taoTests/models/ontology', '*.rdf', 'http://tao.ontology/taoTests');
        ld_dir(f || '/taoGroups/models/ontology', '*.rdf', 'http://tao.ontology/taoGroups');
        ld_dir(f || '/taoSubjects/models/ontology', '*.rdf', 'http://tao.ontology/taoSubjects');
        ld_dir(f || '/taoResults/models/ontology', '*.rdf', 'http://tao.ontology/taoResults');
        ld_dir(f || '/wfEngine/models/ontology', '*.rdf', 'http://tao.ontology/wfEngine');
